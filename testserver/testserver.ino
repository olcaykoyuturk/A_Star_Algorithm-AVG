...
// Grid dimensions and replanning interval
const int GRID_W = 5;
const int GRID_H = 5;
const int LOOP_MS = 1000;  // replanning every 1 second

// Wi-Fi AP credentials
const char* ap_ssid = "AGV_Network";
const char* ap_pass = "12345678";

// AsyncWebServer and WebSocket instances
...
// HTML dashboard and grid UI served from flash
...
<script>
  const W = 5, H = 5;
  let blockedCells = [];    // will be overwritten by server
  let positions = {};
  let targets   = {};
  ...
  ws.onmessage = ev => {
    ...
    if (data.target) {
      targets[data.id] = { x:data.target.x, y:data.target.y };
    }
    renderGrid();
  };
...
// Handle dashboard commands (new target)
...
void loop() {
  ...
  // dynamic blocks: AGV positions + fixed walls
